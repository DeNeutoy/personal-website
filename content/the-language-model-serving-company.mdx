...
Vercel depends on globally distributed CPU servers to provide their high performance SSR serving cache. The "minimumm viable infrastructure" for their SSR feature would be a full, global network of datacenters, so  the capital cost of replicating this functionality with their own infrastructure makes this almost impossible.
...
This is one of the strongest arguments for an OSS model strategy. Mistral's models will proliferate, and companies will run them themselves instead of via Mistral's API. This involves a more substantial investment (time and code) than calling an API, which gives Mistral an edge in retaining users, as well as filtering low value users, as they will likely take the easier route of API usage. Whether they can find services or new models which those users would pay for is another question.
...
LLMs are crying out for structured generation, and there are several open source libraries which accomodate this, either through function calling APIs or grammar based generation.
...
- A javascript LLM analytics library, allowing users to define "events" which correspond to finetuning signals, tied to generations
...
Some of the features here do require direct access to models to be implemented. But I think the majority of them can be abstracted until LLM serving is cheap enough to be run on commodity hardware. Anyway, lots of exciting stuff to build!